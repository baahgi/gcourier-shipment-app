...
export default function OutForDelivery({ navigation }) {
    ...
    useEffect(()=> {
        ...
        axios({
            method: 'GET',
            url: 'https://courier.graphiccourier.com/api/getriders',
    ...
    const updateShipment = () => {
        ...
        if(rider_id){
            ...
            Alert.alert('Failed', 'Select a rider')
        }
            ...
            <ScrollView style={tw`bg-red-50`}>
                ...
                {/* form */}
                    <View style={tw`px-4 py-4 mx-2 my-8 bg-red-100 rounded-lg`}>
                        <Text style={tw`text-xl font-bold text-center text-gray-900 underline`}>Update Out for Delivery</Text>
                        <View style={tw`mt-4`}>
                        ...
                        <View style={tw`mt-6`}>
                            <Text style={tw`text-base font-semibold`}>Select Rider:</Text>
                            <Picker
                            // style={{ height: 50, width: 150 }}
                            selectedValue={rider_id}
                            onValueChange={(itemValue) => setRiderId(itemValue)}
                            style={tw`h-10 px-4 mt-1 text-lg text-gray-700 border-2 border-gray-400 rounded-lg`}
                            >
                                <Picker.Item label="select rider" value="" />
                                {riders.map((rider)=>{
                                    return(
                                        <Picker.Item key={rider.rider_id} label={rider.rider_name} value={rider.rider_id} />
                                    )
                                })}
